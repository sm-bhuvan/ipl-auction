...
import IPLSquadsDisplay from "./IplSquadsDisplay"
...
export default function AuctionPage() {
  ...
  const calculateNextBid = () => {
    if (currentBid === 0) {
      return Number.parseFloat((currentPlayer["Base Price (Rs Lakh)"] / 100).toFixed(2))
    }
    let increment = 0
    if (currentBid < 1) {
    ...
    return Number.parseFloat((currentBid + increment).toFixed(2))
  }
        ...
        try {
          ...
          switch (msg.type) {
            case "joined":
              ...
              setAuctionEnded(msg.auctionEnded || false) // Set auction ended state from server
            ...
            case "auction_ended":
              console.log("Auction ended message received")
            ...
            case "player_sold":
              ...
              setSquads(msg.squads)
              // Don't set auctionEnded here - only when we get auction_ended message
  ...
  const basePriceInCrores = currentPlayer["Base Price (Rs Lakh)"] ? currentPlayer["Base Price (Rs Lakh)"] / 100 : 0
  ...
  // Show squads display if auction has ended
  if (auctionEnded) {
    return <IPLSquadsDisplay squads={squads} />
  }
    ...
    <div className="relative">
      ...
      <div className="absolute top-4 right-4 z-10">
        <div className="flex items-center space-x-2 bg-black/80 backdrop-blur-sm rounded-lg px-3 py-2 shadow-lg">
          <span className="text-xs font-medium text-white/90">Auction Starts in:</span>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-blue-900 p-4">
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border-2 border-orange-500/50 p-8 max-w-2xl w-full text-center">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-4">Auction will start automatically when all teams join</p>
        </div>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                  ...
                  >
                    ...
                    {hoveredTeam === team.code && (
                      <>
                        ...
                        {/* Desktop tooltip - smart positioning to stay within viewport */}
                        <div
                          className={`hidden md:block absolute top-full mt-2 bg-slate-800/95 backdrop-blur-sm border border-white/20 rounded-lg p-4 shadow-xl z-[9998] min-w-[280px] max-w-[350px] max-h-[60vh] overflow-y-auto ${
                            // Check if this is one of the last few teams and position accordingly
                            index >= selectedTeams.length - 2 ? 'right-0' : 'left-0'
                            }`}
                        >
          ...
          <div className="bg-white/10 backdrop-blur-none rounded-lg p-3 border border-white/20">
            ...
            <div className="text-gray-400 text-sm">
              Connected: <span className={`font-semibold ${connectionStatus.color}`}>{connectionStatus.text}</span>
            </div>
          </div>
        </div>
      </header>
            ...
            <div className="p-4 md:p-6 bg-white/5 border-b border-white/20">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                ...
                <div className="text-center p-2 md:p-3 bg-white/10 rounded-lg border border-white/20">
                  <p className="text-xl md:text-2xl font-bold text-white">{currentPlayer["Matches in IPL"] || "0"}</p>
                ...
                {(currentPlayer["Role"]?.includes("Batter") || currentPlayer["Role"]?.includes("Wicketkeeper")) && (
                  <>
                    <div className="text-center p-2 md:p-3 bg-white/10 rounded-lg border border-white/20">
                      <p className="text-xl md:text-2xl font-bold text-white">{currentPlayer["Runs"] || "N/A"}</p>
                ...
                {currentPlayer["Role"]?.includes("Bowler") && !currentPlayer["Role"]?.includes("All-Rounder") && (
                  <>
                    <div className="text-center p-2 md:p-3 bg-white/10 rounded-lg border border-white/20">
                      <p className="text-xl md:text-2xl font-bold text-white">{currentPlayer["Wickets"] || "N/A"}</p>
                    ...
                    <div className="text-center p-2 md:p-3 bg-white/10 rounded-lg border border-white/20">
                      <p className="text-xl md:text-2xl font-bold text-white">{currentPlayer["Economy"] || "N/A"}</p>
            ...
            <div className="p-4 md:p-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                <div className="lg:col-span-2">
                  <CurrentBid current={formatPrice(currentBid)} highestBidder={highestBidder} base={basePriceDisplay} />
                      ...
                      <div className="text-center mb-4 md:mb-6">
                        ...
                        {winningTeam && (
                          <div className="mt-2">
                            ...
                            <p className="text-green-400 font-semibold">Final Price: {formatPrice(finalBid)}</p>
                          </div>
                        )}
                      </div>
                    )
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {biddingHistory.length === 0 ? (
                ...
                biddingHistory.map((bid, index) => (
                  <div
                    key={index}
                    className={`flex items-center justify-between p-2 md:p-3 rounded-lg border ${index === 0 ? "bg-green-600/20 border-green-400/50" : "bg-white/5 border-white/10"
                      }`}
                  >
                    <div className="flex items-center space-x-2 md:space-x-3">
                      <div
                        className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${index === 0 ? "bg-green-400" : "bg-white/50"}`}
                      ></div>
                      <span
                        className={`text-sm md:text-base font-medium ${bid.team === teamName ? "text-orange-400" : "text-white"
                          }`}
                      >
                        {bid.team}
                      </span>
          ...
          <div className="bg-white/10 backdrop-blur-none border border-white/20 rounded-xl p-4 md:p-6 shadow-xl">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {selectedTeams.map((team, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-2 md:p-3 rounded-lg hover:bg-white/5 transition-colors border border-white/10"
                >
                  <div className="flex items-center space-x-2 md:space-x-3">
                    <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full bg-gradient-to-r ${team.color}`}></div>
                    <div>
                      <span
                        className={`text-sm font-medium ${team.code === teamName ? "text-orange-400" : "text-white"}`}
                      >
                        {team.code}
                      </span>
                    ...
                    {team.code === teamName && (
                      <span className="text-xs bg-orange-500 text-white px-1.5 py-0.5 rounded-full">YOU</span>
                    )}
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-gray-400">{formatPrice(teamBudgets[team.code] || 0)}</div>
                  </div>
                </div>
              ))}
            </div>
