...
async function fetchPlayersFromMongo() {
  try {
    ...
    playersFetched = true
    console.log("Total players loaded: ", allPlayers.reduce((sum, role) => sum + role.length, 0))
  } catch (err) {
...
function createInitialSquad() {
  return {
    MI: {
      ...
      Bowler: [{ name: "Jasprit Bumrah", overseas: false }],
      "All-Rounder": [{ name: "Hardik Pandya", overseas: false }],
      "Wicketkeeper-Batter": [],
      Wicketkeeper: [],
    },
    CSK: {
      Batter: [{ name: "Ruturaj Gaikwad", overseas: false }],
      Bowler: [{ name: "Matheesha Pathirana", overseas: true }],
      "All-Rounder": [
        { name: "Ravindra Jadeja", overseas: false },
        { name: "Shivam Dube", overseas: false },
      ],
      "Wicketkeeper-Batter": [],
      Wicketkeeper: [{ name: "MS Dhoni", overseas: false }],
    },
    RCB: {
      Batter: [
      ...
      Bowler: [{ name: "Yash Dayal", overseas: false }],
      "All-Rounder": [],
      "Wicketkeeper-Batter": [],
      Wicketkeeper: [],
    },
    ...
    DC: {
      Batter: [],
      Bowler: [{ name: "Kuldeep Yadav", overseas: false }],
      "All-Rounder": [
        { name: "Axar Patel", overseas: false },
        { name: "Tristan Stubbs", overseas: true },
      ],
      "Wicketkeeper-Batter": [{ name: "Abhishek Porel", overseas: false }],
      Wicketkeeper: [],
    },
    PBKS: {
      Batter: [{ name: "Shashank Singh", overseas: false }],
      Bowler: [],
      "All-Rounder": [],
      "Wicketkeeper-Batter": [{ name: "Prabhsimran Singh", overseas: false }],
      Wicketkeeper: [],
    },
    RR: {
      Batter: [
      ...
      Bowler: [{ name: "Sandeep Sharma", overseas: false }],
      "All-Rounder": [{ name: "Riyan Parag", overseas: false }],
      "Wicketkeeper-Batter": [
    ...
    SRH: {
      ...
      "Wicketkeeper-Batter": [{ name: "Heinrich Klaasen", overseas: true }],
      Wicketkeeper: [],
    },
    GT: {
      Batter: [
      ...
      Bowler: [{ name: "Rashid Khan", overseas: true }],
      "All-Rounder": [
    ...
    LSG: {
      Batter: [{ name: "Ayush Badoni", overseas: false }],
      Bowler: [
      ...
      "All-Rounder": [],
      "Wicketkeeper-Batter": [{ name: "Nicholas Pooran", overseas: true }],
      Wicketkeeper: [],
    },
  }
}
...
const initialTeams = [
  ...
  { name: "Kolkata Knight Riders", code: "KKR", color: "from-purple-600 to-purple-800", logo: "⚔️", initialBudget: 51 },
  { name: "Delhi Capitals", code: "DC", color: "from-blue-500 to-red-500", logo: "🏛️", initialBudget: 73 },
  { name: "Punjab Kings", code: "PBKS", color: "from-red-500 to-yellow-500", logo: "👑", initialBudget: 110.5 },
  { name: "Rajasthan Royals", code: "RR", color: "from-pink-500 to-blue-500", logo: "👑", initialBudget: 41 },
  { name: "Sunrisers Hyderabad", code: "SRH", color: "from-orange-500 to-red-600", logo: "☀️", initialBudget: 45 },
  { name: "Gujarat Titans", code: "GT", color: "from-slate-800 to-yellow-400", logo: "⛰️", initialBudget: 69 },
  { name: "Lucknow Super Giants", code: "LSG", color: "from-blue-400 to-orange-300", logo: "🦅", initialBudget: 69 },
]
...
const removeClientFromRoom = (ws) => {
  ...
  try {
    ...
    if (roomData[ws.roomId] && ws.teamName) {
      const hasOtherClientWithSameTeam = rooms[ws.roomId].some(
        (client) => client.teamName === ws.teamName && client.readyState === WebSocket.OPEN,
      )
...
// Fixed function to check if auction is ended
const isAuctionEnded = (room) => {
  // Check if all role arrays are empty
  if (!room.players || !Array.isArray(room.players)) {
    console.log("No players array found")
    return false
  }

  // Check each role array
  for (let i = 0; i < room.players.length; i++) {
    // If any role has players left, auction is not ended
    if (Array.isArray(room.players[i]) && room.players[i].length > 0) {
      console.log(`Role ${roles[i]} still has ${room.players[i].length} players remaining`)
      return false
    }
  }

  // If we get here, all role arrays are empty
  console.log("All roles are empty - auction should end")
  return true
}

// Add this helper function after the isAuctionEnded function
// This function will find the next non-empty role index
function findNextNonEmptyRoleIndex(room, startIndex) {
  // Check all roles (maximum of roles.length attempts to avoid infinite loop)
  for (let i = 0; i < roles.length; i++) {
    // Get the next role index with wraparound
    const nextIndex = (startIndex + i) % roles.length

    // If this role has players, return its index
    if (room.players[nextIndex] && room.players[nextIndex].length > 0) {
      console.log(`Found next non-empty role: ${roles[nextIndex]} at index ${nextIndex} with ${room.players[nextIndex].length} players`)
      return nextIndex
    }
  }

  // If all roles are empty, return -1 to indicate auction should end
  console.log("No non-empty roles found - auction should end")
  return -1
}
...
const handlePlayerSold = (roomId) => {
  ...
  console.log(`Player sold: ${soldPlayer["Player Name"]} to ${winningTeam || "UNSOLD"} for ₹${finalBid} Cr`)
  ...
  if (room.players[room.currentSetIndex].length > 0) {
    ...
    console.log(`Removed player from ${roles[room.currentSetIndex]}. Players remaining in this role: ${room.players[room.currentSetIndex].length}`)
  }
  ...
  // Check if auction is completely ended FIRST
  if (isAuctionEnded(room)) {
    room.auctionEnded = true
    console.log("🏁 AUCTION ENDED - All players have been sold!")
    broadcastToRoom(roomId, {
      type: "auction_ended",
      room: roomId,
      squads: room.squads,
      finalStats: {
        totalPlayersSold: room.setHistory.length,
        finalBudgets: room.teamBudgets,
      },
    })
    return
  }
  ...
  if (winningTeam && room.teamBudgets) {
    room.teamBudgets[winningTeam] = Number.parseFloat((room.teamBudgets[winningTeam] - finalBid).toFixed(1))
  }

  // Check if we need to rotate set or current set is empty
  ...
  if (shouldRotateSet || currentSetEmpty) {
    // Find the next non-empty role
    const completedSet = roles[room.currentSetIndex]
    const nextSetIndex = findNextNonEmptyRoleIndex(room, (room.currentSetIndex + 1) % roles.length)

    // If there are no more roles with players, end the auction
    if (nextSetIndex === -1) {
      room.auctionEnded = true
      console.log("🏁 AUCTION ENDED - No more players in any role!")
      broadcastToRoom(roomId, {
        type: "auction_ended",
        room: roomId,
        squads: room.squads,
    ...
    // Otherwise, move to the next non-empty role
    room.inBreak = true
    ...
    console.log(`Set break: Completed ${completedSet}, moving to ${roles[nextSetIndex]}`)
...
const moveToNextPlayer = (roomId) => {
  ...
  if (!room.players[room.currentSetIndex] || room.players[room.currentSetIndex].length === 0) {
    console.log("Current set is empty, finding next non-empty set")

    // Find the next non-empty role
    const nextSetIndex = findNextNonEmptyRoleIndex(room, 0)

    // If there are no more roles with players, end the auction
    if (nextSetIndex === -1) {
      room.auctionEnded = true
      console.log("🏁 AUCTION ENDED - All sets are empty!")
      broadcastToRoom(roomId, {
        type: "auction_ended",
        room: roomId,
        squads: room.squads,
  ...
  console.log(`Moving to next player in ${roles[room.currentSetIndex]}. Player index: ${room.currentPlayerInSetIndex + 1}`)
  ...
  broadcastToRoom(roomId, {
    type: "next_player",
    room: roomId,
    index: room.currentPlayerInSetIndex + 1,
    ...
    setInfo: {
      ...
      playersSold: room.currentPlayerInSetIndex,
      totalInSet: room.players[room.currentSetIndex].length + room.currentPlayerInSetIndex,
    },
      ...
      switch (data.type) {
        case "join":
          ...
          if (!rooms[data.room]) {
            ...
            // Log total players available for this room
            const totalPlayers = roomData[data.room].players.reduce((sum, role) => sum + role.length, 0)
          ...
          ws.send(
            JSON.stringify({
              ...
              auctionEnded: roomData[data.room].auctionEnded,
            }),
          )
  ...
  Object.keys(roomData).forEach((roomId) => {
    ...
    if (!room.auctionStarted) {
      ...
      if (room.waitingTeams >= (room.minTeamsRequired || 10)) {
        ...
        console.log(`🚀 Auction started in room ${roomId}! First player: ${room.currentPlayer["Player Name"]}`)
